...
class ProductActionRemoteDataSource
    @Inject
    constructor(
        private val productActionApi: ProductActionApi,
    ) {
        suspend fun addToCart(
            store: String,
            addRequest: ActionRequest,
        ): Response<ActionResponse> = productActionApi.addToCart(store, addRequest)

        suspend fun addToFavorites(
            store: String,
            addRequest: ActionRequest,
        ): Response<ActionResponse> = productActionApi.addToFavorites(store, addRequest)

        suspend fun deleteFromFavorites(
            store: String,
            removeRequest: ActionRequest2,
        ): Response<ActionResponse> = productActionApi.deleteFromFavorites(store, removeRequest)
    }
